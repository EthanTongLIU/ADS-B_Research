...
% -----> 外层循环(信噪比)
SNR = 0; % 信噪比，dB形式(分别取10, 3, 0, -3, -10)
SNR = power(10, SNR / 10); % 信噪比，原始比例形式
sigma = sqrt(alpha^2 / SNR); % 噪声标准差

% -----> 内层循环(beta1)
beta1 = 90; % beta1(分别取20 40 60 80 100 120 140 160
...
for cycletime = 1 : 20000 % 重复进行蒙特卡洛仿真实验
    ...
    % 自相关累积
    sa = zeros(1, length(p) - M + 1);
    for i = 1 : length(p) - M + 1 
        % 自相关累积(4点)
        sa(i) = sqrt((sd(i)*sd(i+1) + sd(i+1)*sd(i+2) + sd(i+2)*sd(i+3) + sd(i+3)*sd(i+4))/4);
    end
    
%     % 单次检验性质
%     for i = 1 : length(sa) - M + 1       
%         % me(i) = mean(abs([sd(i+11:i+11+11-1), sd(i+33:i+33+44-1), sd(i+88:i+88+11-1), sd(i+110:i+110+66-1)])); % 噪声包络均值
%         me(i) = mean(abs([sa(i+11:i+11+11-1), sa(i+33:i+33+44-1), sa(i+88:i+88+11-1), sa(i+110:i+110+66-1)])); % 噪声包络均值
%         sc(i) = pd * sa(i:i+M-1)'; % 滑动求和结果
%         if (sc(i) / me(i) > beta1)
